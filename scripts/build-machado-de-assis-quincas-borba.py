...
def main() -> None:
    ...
    for part_number in sorted(parts.keys(), key=lambda v: int(v)):
        ...
        if numbers:
            toc_lines.append(f"- {format_group_heading(min(numbers), max(numbers))}")
        else:
            toc_lines.append(f'- Parte {part_number}')
