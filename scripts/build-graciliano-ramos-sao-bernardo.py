...
def extract_chapter_number(title: str) -> int | None:
    match = re.search(r'Capítulo\s*—\s*([0-9]{1,3})', title, flags=re.IGNORECASE)
    if not match:
        return None
...
def format_group_heading(start: int, end: int) -> str:
    if start == end:
        return f'Capítulo {start}'
    return f'Capítulos {start:02d} — {end:02d}'
    ...
    *, chapter_title: str, url_abs: str, identifier: str, part_number: str
    ...
    if chapter_number is not None:
        additional_props.insert(
            0, {'@type': 'PropertyValue', 'name': 'chapter-number', 'value': str(chapter_number)}
        )
...
def main() -> None:
    ...
    for part_number in sorted(parts.keys(), key=lambda v: int(v)):
        ...
        nums = [extract_chapter_number(chapter['title']) for chapter in group]
        numbers = [n for n in nums if n is not None]
        if numbers:
            start, end = min(numbers), max(numbers)
            group_label = format_group_heading(start, end)
        else:
            group_label = f'Parte {part_number}'
        toc_lines.append(f'- {group_label}')
    ...
    item_list_ld = {
        ...
        'itemListElement': [
            {'@type': 'ListItem', 'position': idx + 1, 'name': chapter['title'], 'url': chapter['url_abs']}
