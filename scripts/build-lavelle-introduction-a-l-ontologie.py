...
CHAPTER_DIR = Path('src/louis-lavelle/introduction-a-l-ontologie')
OUTPUT_FILE = Path('src/louis-lavelle/introduction-a-l-ontologie.md')
LOCAL_SOURCE_DIR = Path('local-books/louis-lavelle/introduction-a-l-ontologie/fr')

SITE_BASE = 'https://skepvox.com'
BOOK_PATH = '/louis-lavelle/introduction-a-l-ontologie'
BOOK_PATH_HTML = f'{BOOK_PATH}.html'
BOOK_URL = f'{SITE_BASE}{BOOK_PATH_HTML}'
BOOK_TITLE = "Introduction à l'ontologie"
...
def format_intro_title(title: str) -> str:
    lower = title.lower()
    for prefix in ('introduction', 'préface', 'preface'):
...
def part_label(part_number: str) -> str | None:
    labels = {
        '01': 'Première partie',
        '02': 'Deuxième partie',
        '03': 'Troisième partie',
    ...
    return labels.get(part_number)


def parse_identifier(stem: str) -> tuple[str, str, str]:
    parts = stem.split('-', 3)
    if len(parts) < 3:
        sys.exit(f'Unexpected chapter filename: {stem}')
    return parts[0], parts[1], parts[2]


def build_description(chapter_title: str) -> str:
    base = f"Texte intégral en français de {BOOK_TITLE} de {AUTHOR}. {chapter_title}"
...
def build_leaf_frontmatter(local_path: Path) -> str:
    ...
    chapter_title = meta.get('title') or meta.get('chapter-title') or local_path.stem
    ...
    fm_lines = [
        '---',
        f'title: "{escape_yaml(f"{BOOK_TITLE} — {chapter_title}")}"',
        ...
        f'book-title: "{escape_yaml(inferred_part)}"',
        'part-title: ""',
        f'chapter-title: "{escape_yaml(chapter_title)}"',
        'outline: false',
        'head:',
        '  - - link',
        '    - rel: canonical',
        ...
        '  - - meta',
        '    - name: description',
        f'      content: "{escape_yaml(description)}"',
        '  - - meta',
        '    - property: og:title',
        f'      content: "{escape_yaml(f"{BOOK_TITLE} — {chapter_title}")}"',
        '  - - meta',
...
def sync_leaf_bodies(*, create_missing: bool) -> None:
    ...
    for local_path in local_files:
        ...
        if target_path.exists():
            ...
            if not create_missing:
                sys.exit(f'Missing target chapter files in {CHAPTER_DIR}: {local_path.name}')
            target_text = ''
...
def main() -> None:
    ...
    sync_leaf_bodies(create_missing=create_missing)
    ...
    for chapter in chapters:
        ...
        if not book_title:
            if lower_title.startswith(('introduction', 'préface', 'preface', 'avertissement')):
                pending_intro = chapter
                continue
            if pending_intro:
                toc_lines.append('- Avant-propos')
                display_title = format_intro_title(pending_intro['title'])
                toc_lines.append(f"  - [{display_title}]({pending_intro['url_rel']})")
                pending_intro = None
            if lower_title.startswith('conclusion'):
