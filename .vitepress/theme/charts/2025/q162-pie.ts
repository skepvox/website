...
export const renderQ162Pie = (input: ChartRenderInput): ChartInstance => {
  let state = { ...input }
  ...
  const draw = () => {
    ...
    const items =
      payload.items || [
        { label: 'Futebol', value: 180 },
        { label: 'Vôlei', value: 100 },
        { label: 'Basquete', value: 80, valueLabel: '80' }
      ]
    ...
    const layoutGrow = 8
    const margin = {
      top: Math.max(0, 12 + titleHeight + 20 - layoutGrow - titleOffset),
      right: 16,
      bottom: Math.max(0, 12 + legendBlockHeight - layoutGrow),
      left: 16
    }
    ...
    if (titleLines.length > 0) {
      titleLines.forEach((line, index) => {
        svg
          .append('text')
          .attr('x', width / 2)
          .attr('y', 12 + 20 - titleOffset + index * titleLineHeight)
    ...
    chart
      ...
      .append('path')
      .attr('d', arc)
      .attr('fill', (d, index) => d.data.color || DEFAULT_COLORS[index % DEFAULT_COLORS.length])
      .attr('stroke', '#ffffff')
      .attr('stroke-width', 1)

    chart
      .selectAll('.value-label')
      .data(arcs.filter((d) => d.data.valueLabel))
      .enter()
      .append('text')
